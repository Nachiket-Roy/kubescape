...
**What this bounds:** with the scan-scoped partition store, namespaced spill usage and partition-store metadata stay flat (~1-2 MB regardless of cluster size), bounding the *collection-phase memory peak* to the resident batch plus the active pager buffer. Streaming also bounds the *evaluation input* and the *number of API-server calls*. Downstream stages access resources on demand via `ResourceCatalog`, and whole-cluster controls are bounded by execution policies (see below).
...
Execution is governed by `--whole-cluster-policy` / `KUBESCAPE_WHOLE_CLUSTER_POLICY`, with four modes:
...
4. **`verify`** (`KUBESCAPE_WHOLE_CLUSTER_PARITY_CHECK=true`) — debug cross-check mode: runs whole-cluster controls against both `projected` and `fallback` scopes and reports any verdict divergence.
...
Since `scope.resident` holds all cluster-scoped resources, rules can access them regardless of which namespace batch is currently being processed.
...
Collection-phase memory claims (flat ~1-2 MB spill/metadata baseline across cluster sizes) are verified across 5k, 20k, and 50k synthetic clusters with 50-200 resident nodes in `BenchmarkStreamingCollectorMemory`.

## Performance Impact

**API-server calls:**
- Small clusters (<2500 resources): no change (single-batch mode).
- Large clusters (>2500 resources): LIST calls drop from O(L × N) to O(L). For a cluster with 2500 namespaces and 100 GVRs, that's ~250,000 LIST calls down to 100.
...
| `KUBESCAPE_WHOLE_CLUSTER_PARITY_CHECK` | `false` | Set `true` to run the `verify` cross-check mode |
...
- In constrained environments (e.g. operator pods with `readOnlyRootFilesystem: true` and no writable `/tmp`), if the spill directory can't be created, the collector logs a warning and falls back automatically to `partitionstore.NewMemoryStore()`.
...
- Backward compatible: clusters below the threshold keep using the single-batch approach.
- Opt-in / opt-out via `--enable-streaming`.
- Threshold adjustable via `LARGE_CLUSTER_SIZE`.
- No breaking changes for existing behavior below the threshold.

## Conclusion

Collection uses a single LIST pass per GVR instead of one per GVR per namespace, and namespace evaluation is streamed so the OPA processor never sees the whole cluster at once during per-namespace evaluation. With the partition store integrated, the collection-phase memory peak is bounded to the resident batch plus a flat ~1-2 MB spill baseline. Downstream stages access resources on demand via `ResourceCatalog`, and whole-cluster controls default to a bounded projected working set (with explicit full-cluster fallback and parity verify modes when needed) — preserving cross-namespace join fidelity without unbounded memory growth.
